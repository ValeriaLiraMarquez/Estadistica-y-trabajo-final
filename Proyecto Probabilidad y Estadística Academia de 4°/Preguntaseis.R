...
#Leer el documento
texto <- readLines("Q6.txt")
texto = iconv(texto, to="ASCII//TRANSLIT")
texto_c<-Corpus(VectorSource(texto))

# Poner todos los datos a minuscula 
pre6<-tm_map(texto_c, tolower)


# Quitar espacios en blanco
pre6<-tm_map(pre6, stripWhitespace)


# Quitar la puntuacion 
pre6<-tm_map(pre6, removePunctuation)


# Quitar los numeros 
pre6 = tm_map(pre6, removeNumbers)
...
pre6 = tm_map(pre6, removeWords,stopwords("spanish"))
...
letras6= TermDocumentMatrix(pre6)
findFreqTerms(letras6, lowfreq=5)
matrix=as.matrix(letras6)
