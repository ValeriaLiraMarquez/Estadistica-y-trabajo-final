...
#Leer el documento
texto <- readLines("Q9.txt")
texto = iconv(texto, to="ASCII//TRANSLIT")
texto_c<-Corpus(VectorSource(texto))

# Poner todos los datos a minuscula 
pre9<-tm_map(texto_c, tolower)


# Quitar espacios en blanco
pre9<-tm_map(pre9, stripWhitespace)


# Quitar la puntuacion 
pre9<-tm_map(pre9, removePunctuation)


# Quitar los numeros 
pre9 = tm_map(pre9, removeNumbers)
...
pre9 = tm_map(pre9, removeWords,stopwords("spanish"))
...
letras9= TermDocumentMatrix(pre9)
findFreqTerms(letras9, lowfreq=5)
matrix=as.matrix(letras9)
