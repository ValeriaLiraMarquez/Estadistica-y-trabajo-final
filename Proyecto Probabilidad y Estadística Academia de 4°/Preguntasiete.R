...
#Leer el documento
texto <- readLines("Q7.txt")
texto = iconv(texto, to="ASCII//TRANSLIT")
texto_c<-Corpus(VectorSource(texto))

# Poner todos los datos a minuscula 
pre7<-tm_map(texto_c, tolower)


# Quitar espacios en blanco
pre7<-tm_map(pre7, stripWhitespace)


# Quitar la puntuacion 
pre7<-tm_map(pre7, removePunctuation)


# Quitar los numeros 
pre7 = tm_map(pre7, removeNumbers)
...
pre7 = tm_map(pre7, removeWords,stopwords("spanish"))
...
letras7= TermDocumentMatrix(pre7)
findFreqTerms(letras7, lowfreq=5)
matrix=as.matrix(letras7)
