...
color=c("darkslateblue","dodgerblue3")
...
# 1.- Creación de un vector de color
color=c("blue1", "darkcyan", "aquamarine")

# 2.- Creación del gráfico
GB1<-ggplot(penguins, aes(x=año))+
  geom_bar(colour= "blueviolet", fill=color)+
...
GB2<-ggplot(penguins, aes(x=año))+
  geom_bar(colour= "darkslategrey", fill=color)+
  ggtitle("Gráfico de Barras")+
  xlab("Año")+
  ylab("Frecuencias")+
  coord_flip()+
...
GD3<-ggplot(penguins, aes(x=largo_pico_mm, y=grosor_pico_mm))+
  geom_point(aes(color=especie))+
  scale_color_manual (values=c("pink","lightblue1","gold"))+
  ggtitle("Gráfico de dispersión")+
  xlab("largo del pico (mm)")+
  ylab("grosor del pico (mm)")+
  theme_light()
