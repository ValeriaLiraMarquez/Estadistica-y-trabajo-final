...
#Leer el documento
texto <- readLines("Q10.txt")
texto = iconv(texto, to="ASCII//TRANSLIT")
texto_c<-Corpus(VectorSource(texto))

# Poner todos los datos a minuscula 
pre10<-tm_map(texto_c, tolower)


# Quitar espacios en blanco
pre10<-tm_map(pre10, stripWhitespace)


# Quitar la puntuacion 
pre10<-tm_map(pre10, removePunctuation)


# Quitar los numeros 
pre10 = tm_map(pre10, removeNumbers)
...
pre10 = tm_map(pre10, removeWords,stopwords("spanish"))
...
letras10= TermDocumentMatrix(pre10)
findFreqTerms(letras10, lowfreq=5)
matrix=as.matrix(letras10)
